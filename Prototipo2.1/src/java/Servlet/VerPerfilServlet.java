...
public class VerPerfilServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String aliasU = request.getParameter("AliasU");
        Usuario usuario;
        if(aliasU == null){
            usuario = (Usuario) request.getSession().getAttribute("User");
        }
        else{
            usuario = UsuarioBD.obtenerUsuarioNombre(aliasU);
        }
        request.setAttribute("usuario", usuario);
        ArrayList<Valoracion> valoraciones;
        valoraciones = ValoracionDB.getValoracionUsuario(usuario.getAliasPropio());
